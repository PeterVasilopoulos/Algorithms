...
Do not include key value pairs from the given objects prototype
(these are included by default when looping over an object's keys)
*/
...
function entries(obj) {
    // Declare new array to hold the keys value pair arrays
    const bigArray = []

    // Need a way to iterate through the object
    for(key in obj) {
        if(obj.hasOwnProperty(key)) {
            // Push array of each key value pair into big array
            bigArray.push([key, obj[key]])
        }
    }
